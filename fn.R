...
recodeCTQ <- function() {
  ...
  #creating Emotional Abuse scale = Items 3, 8, 14, 18, 25*
    MAPS <- mutate(MAPS, CTQ_EA_total= MAPS$T2_CTQ_3 +  MAPS$T2_CTQ_8 +  MAPS$T2_CTQ_14 +  MAPS$T2_CTQ_18 +  MAPS$T2_CTQ_25)
 
  #creating Physical Abuse scale = Items 9, 11, 12, 15, 17*
    MAPS <- mutate(MAPS, CTQ_PA_total= T2_CTQ_9 + T2_CTQ_11 + T2_CTQ_12 + T2_CTQ_15 + T2_CTQ_17)

  #creating Sexual Abuse scale = Items 20, 21, 23, 24, 27*
    MAPS <- mutate(MAPS, CTQ_SA_total= T2_CTQ_20 + T2_CTQ_21 + T2_CTQ_23 + T2_CTQ_24 + T2_CTQ_27)
  ...
  #creating Emotional Neglect scale = Items 5 (reverse scored), 7 (reverse scored), 13 (reverse scored), 19 (reverse scored), 28 (reverse scored)*
    MAPS <- mutate(MAPS, CTQ_EN_total= as.numeric(T2_CTQ_5r) + as.numeric(T2_CTQ_7r) + as.numeric(T2_CTQ_13r) + as.numeric(T2_CTQ_19r) + as.numeric(T2_CTQ_28r))
  
  #creating Physical Neglect scale = Items 1, 2 (reverse scored), 4, 6, 26 (reverse scored)*
    MAPS <- mutate(MAPS, CTQ_PN_total= T2_CTQ_1 + as.numeric(T2_CTQ_2r) + T2_CTQ_4 + T2_CTQ_6 + as.numeric(T2_CTQ_26r))
 
  #creating total score
    MAPS <- mutate(MAPS, CTQ_total= CTQ_EA_total + CTQ_PA_total + CTQ_SA_total+ CTQ_EN_total + CTQ_PN_total)
 

  #T1 CTQ 
  #T1 CTQ Omits Items: 4, 9, 11, 12, 15, 17, 20, 21, 23, 24, 25, 27****
  #Consequently, the Physical Abuse & Sexual Abuse subscales wil be omitted also**
  #reverse scoring items items 2, 5, 7, 13, 19, 26, 28*
  ...
  #creating total score
    MAPS <- mutate(MAPS, CTQ, T1CTQtot= T1_CTQ_EA_total + T1_CTQ_EN_total + T1_CTQ_PN_total)
